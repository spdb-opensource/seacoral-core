...
deleteMappingView () {
    ...
    local url="https://${san_ip}:${san_port}/deviceManager/rest/${device_id}/mappingview/${mappingview_id}"
    local response
...
deleteMappingViewHostGroupAssociate () {
    local san_ip="${SAN_IP}"
    local san_port="${SAN_PORT}"
    local device_id="${SESSION_DEVICE_ID}"
    local ibasetoken="${SESSION_IBASETOKEN}"
    ...
    local url="https://${san_ip}:${san_port}/deviceManager/rest/${device_id}/mappingview/remove_associate"
...
deleteMappingViewLunGroupAssociate () {
    local san_ip="${SAN_IP}"
    local san_port="${SAN_PORT}"
    local device_id="${SESSION_DEVICE_ID}"
    local ibasetoken="${SESSION_IBASETOKEN}"
...
main () {
    ...
    #check mappingview is existed or not . If existed , get mappingview id
    ...
    if getValueNotNull ".data[0]" "${response}" &> /dev/null; then
        ...
        sanLogout
        die 0 "mappingview ${MAPPINGVIEW_NAME} is not existed , no need to delete"
    fi
    ...
    if getValueNotNull ".data[0]" "${response}" &> /dev/null; then
        ...
        if [[ "${hostgroup_id}" == "${hostgroup_id_match}" ]]; then
            ...
            sanLogout "${SAN_IP}" "${SAN_PORT}" "${device_id}" "${ibasetoken}"
    ...
    #delete hostgroup association to mappingview
    if [[ "${hostgroup_is_mapping}" == true ]]; then
        response="$( deleteMappingViewHostGroupAssociate "${hostgroup_id}" "${mappingview_id}" )"
        ERROR_DESC="$( checkResponse "${response}" ".error.code" ".error.description" )" || {
            ERROR_CODE=$?
            sanLogout
            die "${ERROR_CODE}" "deleteMappingViewHostGroupAssociate : ${ERROR_DESC}"
        }
        echo "delete hostgroup ${HOSTGROUP_NAME} to mappingview ${MAPPINGVIEW_NAME} success!"
    else
        echo "hostgroup ${HOSTGROUP_NAME} is not mapping!"
    fi

    #delete lungroup association to mappinview
    if [[ "${lungroup_is_mapping}" == true ]]; then
        response="$( deleteMappingViewLunGroupAssociate "${lungroup_id}" "${mappingview_id}" )"
        ERROR_DESC="$( checkResponse "${response}" ".error.code" ".error.description" )" || {
            ERROR_CODE=$?
            sanLogout
            die "${ERROR_CODE}" "deleteMappingViewLunGroupAssociate : ${ERROR_DESC}"
        }
        echo "delete lungroup ${LUNGROUP_NAME} to mappingview ${MAPPINGVIEW_NAME} success!"
    else
        echo "lungroup ${LUNGROUP_NAME} is not mapping"
    fi

    #delete mappingview
    response="$( deleteMappingView "${mappingview_id}" )"
    ERROR_DESC="$( checkResponse "${response}" ".error.code" ".error.description" )"|| {
        ERROR_CODE=$?
        sanLogout
        die "${ERROR_CODE}" "deleteMappingView : ${ERROR_DESC}"
    }
    echo "delete mappingview ${MAPPINGVIEW_NAME} success!"

    sanLogout
    echo "delete mapping done"
}
