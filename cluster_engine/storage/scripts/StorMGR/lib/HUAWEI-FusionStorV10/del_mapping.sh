...
HOSTGROUP_NAME="$(getValueNotNull ".data.hostgroup_name" "${INPUT}")" || die $? "HOSTGROUP_NAME is null!"
...
deleteMappingView () {
    ...
    local url="https://${san_ip}:${san_port}/${basic_uri}/${version}/volume/detach"
...
main () {
    ...
    for ((i=0;i<${#storagepool_ids[@]};i++)); do
        ...
        if getValueNotNull ".volumeList[0]" "${response}"; then
            local j=0
            while getValueNotNull ".volumeList[${j}]" "${response}"; do
                local lun_names
    ...
    for lun_name in ${lun_names[*]}; do
        ...
        if [[ "$(getValueNotNull ".volAttachNodeInfo[0].nodeIp" "${response}")" == "${HOSTGROUP_NAME}" ]]; then
            local lun_names_match
            lun_names_match[${count}]=${lun_name}
            match="true"
            ((count++))
        else
            echo "lun ${lun_name} is not associated to hostgroup ${HOSTGROUP_NAME}, no need to delete!"
        fi
    done
    ...
    if [[ "${match}" == "true" ]]; then
        ...
        local response
        response="$(deleteMappingView "${SAN_IP}" "${SAN_PORT}" "${BASIC_URI}" "${version}" "${x_auth_token}" "${HOSTGROUP_NAME}" "${lun_name_list}")"
        local output
        output="$(checkResponse "${response}" ".result" ".description")"
        local ret=$?
        if [[ ${ret} -ne 0 ]]; then
        ...
        echo "delete lun ${lun_name_list} to hostgroup ${HOSTGROUP_NAME} success!"
    fi
