...
main () {
    ...
    local url="http://${SAN_IP}:${SAN_PORT}/lungroup/expand"
    ...
    if [[ "${ERROR_CODE}" -eq 5 ]]; then
        echo "lungroup ${LUNGROUP_NAME} is already expanded, no need to expand"
    elif [[ "${ERROR_CODE}" -eq 0 ]]; then
        echo "expand lungroup ${LUNGROUP_NAME} success!"
    else
