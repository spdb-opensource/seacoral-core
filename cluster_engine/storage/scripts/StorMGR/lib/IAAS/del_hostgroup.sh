...
main () {
    ...
    local url="http://${SAN_IP}:${SAN_PORT}/hostgroup/delete"
    ...
    if [[ "${ERROR_CODE}" -eq 5 ]]; then
        echo "hostgroup ${HOSTGROUP_NAME} is not existed, no need to delete"
    elif [[ "${ERROR_CODE}" -eq 0 ]]; then
        echo "del hostgroup ${HOSTGROUP_NAME} success!"
    else
