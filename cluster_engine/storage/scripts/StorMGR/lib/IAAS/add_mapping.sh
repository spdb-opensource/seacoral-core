...
main () {
    ...
    local url="http://${SAN_IP}:${SAN_PORT}/lungroup/add_mapping"
    ...
    if [[ "${ERROR_CODE}" -eq 5 ]]; then
        echo "hostgroup ${HOSTGROUP_NAME} is associated to lungroup ${LUNGROUP_NAME}"
    elif [[ "${ERROR_CODE}" -eq 0 ]]; then
        echo "create associate hostgroup ${HOSTGROUP_NAME} to lungroup ${LUNGROUP_NAME} success!"
    else
